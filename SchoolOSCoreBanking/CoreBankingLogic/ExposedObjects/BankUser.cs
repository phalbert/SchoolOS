...
public class BankUser : BaseObject
{
    ...
    public string Pin = "";
    public bool RestPin = false;
    public bool RestPassword = false;
    ...
    public BankUser(BankUser user)
    {
        ...
        this.RestPassword = user.RestPassword;
        this.RestPin = user.RestPin;
    }
    ...
    public bool IsValid(string bankCode, string Password)
    {
        ...
        this.TransactionLimit = bll.RemoveCommasFromMoneyString(this.TransactionLimit);
        ...
        if ((Usertype.ToUpper() == "TELLER" || Usertype.ToUpper() == "SUPERVISOR") && string.IsNullOrEmpty(TransactionLimit))
        {
            StatusCode = "100";
            StatusDesc = "PLEASE SUPPLY A TRANSACTION LIMIT FOR THIS TELLER OR SUPERVISOR";
            return false;
        }
        //has the user set a transaction limit that exceeds that set for the newUsers branch
        if (bll.ViolatesBranchTransactionLimits(this, out valObj))
        {
            StatusCode = "100";
            StatusDesc = valObj.StatusDesc;
            return false;
        }
        if (!bll.IsValidUser(this.ModifiedBy, BankCode, "SYS_ADMIN|BANK_ADMIN|CUSTOMER_SERVICE", out valObj) && this.ApprovedBy.ToUpper()!="ADMIN")
        {
